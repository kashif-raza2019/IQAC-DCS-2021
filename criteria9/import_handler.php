...
<?php
  ...
  if($file==NULL){
    ...
    $targetPath = "uploaded_csv/"."(".$_SESSION['username'].")_(".$_POST['section'].")_(".$tt.")___".basename($_FILES['csv_file']['name']); // Target path where file is to be stored
  ...
  //  echo "Dept : ".$_POST['dept']."<br>";
    ...
    $query = "DELETE FROM ".$_POST['section']." WHERE username LIKE '".$_SESSION['username']."'";
...
//      echo $query;
    ...
    if($line>=1){
      ...
      save_log($_SESSION['username'],getUserIP(),$_SERVER['REQUEST_URI'],"USER DATA  IMPORT FAILED",date_format($date, 'Y-m-d H:i:s'));

  }

  if($err!=1){
    echo "<a style='color:green;'>".$line." Rows SUCCESSFULLY IMPORTED</a><br>";
//    echo "<a style='color:green;'>DATA SUCCESSFULLY IMPORTED</a><br>";
    echo "<a href='c9.php'>Extended Profile</a><br>";

    save_log($_SESSION['username'],getUserIP(),$_SERVER['REQUEST_URI'],"USER DATA  IMPORT SUCCESSFUL",date_format($date, 'Y-m-d H:i:s'));
  }

...
<body>
    ...
    <br>
    <h3 style="text-align: center; font-weight: bold;">FEEDBACK REPORT | MALE STUDENTS</h3><br>
                ...
                <?php
                    $getQuery1 = mysqli_query($connection, "SELECT*FROM admins ORDER BY name DESC");
                          while($fetchData = $getQuery1->fetch_assoc()){
                              ...
                              $name = $fetchData['name'];
                        $getQuery = mysqli_query($connection, "SELECT*FROM feedback_student_male_2021 WHERE username = '$username'");
                        $studentCount = mysqli_num_rows($getQuery);
                        echo "<tr><td style='color: #0E3778; font-weight: bold' class='deptName'>$name</td><td style='font-weight: bold; text-align: center;'>$studentCount</td><td><a href='report.php?user=$username&type='student_male''><i class='fa fa-file-pdf-o' style='font-size: 26px; color: red;'></i></a></td><td><a href='feedback_student-csv.php?=$username'><i class='fa fa-file-excel-o' style='font-size: 26px; color: green;'></i></a></td></tr>";
                    }

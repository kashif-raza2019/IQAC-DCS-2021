...
<body>
    ...
    <div class="tableContainer">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th colspan="9">STUDENT's SURVEY REPORT</th>
                </tr>
                <tr>
                    <th>S.No</th>
                    <th>GENDER</th>
                    <th>YEAR</th>
                    <th>ANALYSE SURVEY</th>
                </tr>
                    
            </thead>
            <tbody>
                <?php
                  $getData = mysqli_query($connection, "SELECT*FROM sss_2021 WHERE username = '$user'");
                  $i = 0;
                  while($getSt = $getData->fetch_assoc()){
                      $i++;
                      $getid = $getSt['id'];
                      $getgender = $getSt['student_gender'];
                      $getyear = $getSt['student_year'];
                      echo "<tr><td>$i</td><td>$getgender</td><td>$getyear</td><td><a href='view.php?id=$getid' class='btn btn-primary'>View</a></td></tr>";
                  }
                ?>

            </tbody>
        </table>
        <button type="button" onclick="window.print();" style="margin-left: 40px; margin-top: 10px; margin-bottom:" class="btn btn-info">PRINT</button>
    </div>

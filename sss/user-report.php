...
<body>
    ...
    <br>
    <h3 style="text-align: center; font-weight: bold;">Student Satisfaction Survey</h3><br>
    <!-- Main Page Startes From Here -->
    <div class="tableContainer">
        <table class="table table-bordered">
            <thead>
                    <th>DEPARTMENT NAME</th>
                    <th>STUDENTS SURVEYED</th>
                    <th>CSV DATA</th>
            </thead>
            <tbody>
                <?php
                    $getQuery = mysqli_query($connection, "SELECT*FROM admins ORDER BY username DESC");
                    while($getAdmin = $getQuery->fetch_assoc()){
                        $getAdminData = $getAdmin['username'];
                        $getAdminName = $getAdmin['name'];
                        $getSSSQuery = mysqli_query($connection, "SELECT*FROM sss_2021 WHERE username = '$getAdminData'");
                        $studentCount = mysqli_num_rows($getSSSQuery);
                        echo "<tr><td style='color: #0E3778; font-weight: bold' class='deptName'>$getAdminName</td><td style='font-weight: bold; text-align: center;'>$studentCount</td><td><a href='sss-csv.php?user=$getAdminData&name=$getAdminName'><i class='fa fa-file-excel-o' style='font-size: 26px; color: green;'></i></a></td></tr>";
                    }

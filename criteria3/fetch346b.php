...
//save_log($_SESSION['username'],getUserIP(),$_SERVER['REQUEST_URI'],urlencode(http_build_query($_POST, '', '&amp;')),date_format($date, 'Y-m-d H:i:s'));
	...
	$query = "select distinct * from t3_4_6b where Username like '".$_SESSION['username']."';";
	...
	while($row = $res->fetch_assoc()){
        echo '<tr id = "'.$row['id'].'"><td><center><input type="text" value="'. $row['tname'].'" placeholder="Name" style="width:200px;" required></center></td>'.
              '<td><center><input type="text" value="'. $row['aname'].'" placeholder="Name of Authors" style="width:200px;" required></center></td>'.
              '<td><center><input type="text" value="'. $row['ptitle'].'" placeholder="Paper Title" style="width:200px;" required></center></td>'.
              '<td><center><input type="text" value="'. $row['prestype'].'" placeholder="Type of Presentation" style="width:200px;" required></center></td>'.
              '<td><center><input type="text" value="'. $row['cname'].'" placeholder="Conference Name" style="width:200px;" required></center></td>'.

...
//save_log($_SESSION['username'],getUserIP(),$_SERVER['REQUEST_URI'],urlencode(http_build_query($_POST, '', '&amp;')),date_format($date, 'Y-m-d H:i:s'));
	...
	$query = "select distinct * from t3_4_7 where Username like '".$_SESSION['username']."';";
	...
	while($row = $res->fetch_assoc()){
        ...
        echo '<tr id = "'.$row["id_time"].'"><td><center><input type="text" value = "'.$row["PTitle"].'" placeholder="Paper Title" style="width:200px;" required></center></td>'.
              ...
              '<td><center><input type="text" placeholder="citation index" value = "'.$row["indx"].'" style="width:200px;" required></center></td>'.

...
//save_log($_SESSION['username'],getUserIP(),$_SERVER['REQUEST_URI'],urlencode(http_build_query($_POST, '', '&amp;')),date_format($date, 'Y-m-d H:i:s'));
	...
	$query = "select distinct * from t3_5_2 where Username like '".$_SESSION['username']."';";
	...
	while($row = $res->fetch_assoc()){
        ...
        echo '<tr id = "'.$row["id_time"].'"><td><center><input type="text" value = "'.$row["Name"].'"  placeholder="Name" style="width:200px;" required></center></td>'.
              '<td><center><input type="text" placeholder="project" value = "'.$row["Project"].'"  style="width:200px;" required></center></td>'.
